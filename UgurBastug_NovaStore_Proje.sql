...
CREATE TABLE orders (
    ...
    order_date TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_DATE,
...
SELECT full_name, product_name, price, category_name from products JOIN categories ON categories.category_id = products.category_id
JOIN order_details ON products.product_id = order_details.product_id
JOIN orders ON order_details.order_id = orders.order_id
JOIN customers ON orders.customer_id = customers.customer_id WHERE full_name = 'Ahmet Yılmaz';

-- Sorgu 4: Hangi kategoride toplam kac adet urun var
SELECT categories.category_name, SUM(stock) FROM products JOIN categories ON categories.category_id = products.category_id
GROUP BY categories.category_name;

-- Sorgu 5: Her musterinin sirkete kazandirdigi toplam ciro
SELECT full_name, SUM(total_amount) AS total_spent FROM customers JOIN orders ON customers.customer_id = orders.customer_id
GROUP BY customers.full_name ORDER BY total_spent desc;
...
SELECT full_name, order_date, product_name, quantity FROM customers 
